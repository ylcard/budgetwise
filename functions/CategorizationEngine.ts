...
// Simple delay to respect rate limits if we are looping fast
const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));

// Default to 70b because it has 12k TPM vs 8b's 6k TPM
async function matchWithAI(descriptions, categories, groqKey, model = 'llama-3.3-70b-versatile') {
  if (!groqKey || descriptions.length === 0) return {};

  // Deduplicate to save tokens and avoid JSON key collisions
  ...
  const prompt = `Act as a financial data cleaner.
    ...
    Merchants to categorize: ${JSON.stringify(uniqueDescriptions)}`;
  ...
  try {
    ...
    if (response.status === 429) {
      console.warn(`Groq Rate Limit Hit (TPM). Retrying after delay...`);
      await sleep(2000); // Wait 2s
      return matchWithAI(descriptions, categories, groqKey, model);
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (toAI.length > 0) {
      ...
      let aiMappings = await matchWithAI(descriptions, categories || [], groqKey); // Use default 70b
      ...
      if (lowConfidenceItems.length > 0) {
        try {
          ...
          console.warn("Tier 2 classification failed, sticking with Tier 1 results", e);
        }
      }
