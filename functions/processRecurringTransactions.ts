      ...
      try {
        ...
        while (
          ...
          if (recurring.type === 'expense' && recurring.financial_priority) {
            ...
            // Fetch only the specific budget needed for this user/month
            const matchingBudgets = await base44.asServiceRole.entities.SystemBudget.filter({
              user_email: recurring.user_email,
              systemBudgetType: recurring.financial_priority,
              startDate: monthStart,
              endDate: monthEnd
            });

            const matchingBudget = matchingBudgets[0];
