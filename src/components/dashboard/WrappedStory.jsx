...
import { X, Download, TrendingUp, TrendingDown, Award, Calendar, ChevronDown, ChevronUp, BarChart2, ArrowRight } from "lucide-react";
import { formatCurrency } from "../utils/currencyUtils";
import { BudgetAvatar } from "../ui/BudgetAvatar"; // Re-using your ghost!
import html2canvas from "html2canvas";
import { jsPDF } from "jspdf";
import { getMonthBoundaries, getMonthName, isDateInRange } from "../utils/dateUtils";
...
/**
 * Custom hook to calculate top 10 categories based on actual transaction data.
 * @param {Array} transactions - List of transactions for the period
 * @param {Array} categories - List of available categories
 * @returns {Array} Sorted list of top 10 categories with amount and percentage
 */
...
/**
 * WrappedStory Component
 * A mobile-first, story-style recap of the user's financial month.
 * Features progress bars, auto-advancing slides, and a downloadable PDF summary.
 * @param {Object} props
 * @param {boolean} props.isOpen - Visibility state
 * @param {Function} props.onClose - Close handler
 * @param {number} props.month - Target month (0-11)
 * @param {number} props.year - Target year
 */
export const WrappedStory = ({
  ...
  const storyTransactions = useMemo(() => {
    return transactions.filter(t => isDateInRange(t.date, monthStart, monthEnd));
  ...
  const monthName = useMemo(() =>
    month !== undefined ? getMonthName(month) : '',
    [month]);
  ...
  const heavyHittersRef = useRef(null);
