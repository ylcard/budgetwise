...
const RemainingBudgetCard = memo(function RemainingBudgetCard({
  ...
  settings,
  monthNavigator,
  // REMOVED 10-Mar-2026: Button props moved to QuickActions component in the right column
  // addIncomeButton,
  // addExpenseButton,
  // importDataButton,
  // syncButton,
  ...
  // Clamped for progress bar logic but used raw for the header text
  const savingsAmount = Math.max(0, netBalance);
  const extraSavingsAmount = Math.max(0, netBalance - savingsLimit);

  // Calculate display percentage based on safeIncome
  // Use the absolute value of netBalance for "Over Budget" scenarios
      ...
      <>
        ...
        >
          ...
          >
            ...
            >
              ...
              {!isSimpleView && (
                <div className="w-full px-1 flex items-center justify-center">
                  {/* Primary Label (Left) */}
                  {nR.p > 0.1 && (
                    <div className="text-[11px] sm:text-xs font-bold text-white dark:text-black overflow-hidden whitespace-nowrap drop-shadow-sm">
                      <TextSwap>
            ...
            >
              {!isSimpleView && (
                <div className="text-[11px] sm:text-xs font-bold text-white dark:text-black px-1 whitespace-nowrap drop-shadow-sm">
                  <TextSwap>
          ...
          >
            ...
            >
              ...
              {!isSimpleView && (
                <div className="w-full px-1 flex items-center justify-center">
                  {wR.p > 0.1 && (
                    <div className="text-[11px] sm:text-xs font-bold text-white dark:text-black whitespace-nowrap drop-shadow-sm">
                      <TextSwap>
            ...
            >
              {!isSimpleView && (
                <div className="text-[11px] sm:text-xs font-bold text-white dark:text-black whitespace-nowrap px-1 drop-shadow-sm">
                  <TextSwap>
        ...
        {projectedExpenseOuterPct > 0 && !isSimpleView && (
          ...
          >
            <SmartSegment
              ...
              direction="center"
            >
              <div className="w-full px-1 flex items-center justify-center text-[10px] sm:text-[11px] font-bold text-foreground whitespace-nowrap">
                <TextSwap>
          ...
          >
            ...
            >
              ...
              {!isSimpleView && (
                <div className="w-full px-1 flex items-center justify-center">
                  {sTargetRatio > 0.1 && (
                    <div className="text-[11px] sm:text-xs font-bold text-white dark:text-black truncate px-1 drop-shadow-sm">
                      <TextSwap>
                  ...
                  {sExtraRatio <= 0.15 && sTargetRatio > 0.15 && sExtra > 0 && (
                    <div className="text-[10px] font-medium text-white/90 dark:text-black/90 whitespace-nowrap pl-1 drop-shadow-sm">
            ...
            >
              {!isSimpleView && (
                <div className="text-[11px] sm:text-xs font-bold text-[hsl(var(--stat-income-text))] truncate px-1">
                  <TextSwap>
        ...
        <div className="flex flex-col gap-6">
          {isEmptyMonth ? (
            <div className="flex flex-col items-center justify-center py-10 text-center space-y-5 animate-in fade-in zoom-in-95 duration-500">
              <div className="w-16 h-16 bg-[hsl(var(--stat-income-bg))] rounded-full flex items-center justify-center shadow-sm">
                ...
                <div>
                  {isActuallyOver ? (
                    <div className="space-y-1">
                      <h2 className="text-4xl font-extrabold text-destructive flex items-center gap-2 tracking-tight">
                        {Math.round(absPctDisplay)}% <span className="text-xl font-semibold opacity-90">{monthStatus === 'current' ? "Projected over" : "Over budget"}</span>
                      </h2>
                      <p className="text-sm font-medium text-destructive flex items-center gap-1.5">
                        <AlertCircle className="w-4 h-4" /> Careful! You are {formatCurrency(Math.abs(netBalance), settings)} over your limit.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-1">
                      <h2 className="text-4xl font-extrabold text-foreground flex items-center gap-2 tracking-tight">
                        {Math.round(absPctDisplay)}% <span className="text-xl font-semibold text-[hsl(var(--stat-income-text))]">{monthStatus === 'current' ? "Projected to save" : "Saved"}</span>
                      </h2>
                      <p className="text-sm font-medium text-muted-foreground flex items-center gap-1.5 animate-in fade-in slide-in-from-left-2 duration-500">
                        {extraSavingsAmount > 0 ? (
                          <><TrendingUp className="w-4 h-4 text-[hsl(var(--stat-income-text))]" /> You've crushed your goal by <span className="text-[hsl(var(--stat-income-text))] font-bold">{formatCurrency(extraSavingsAmount, settings)}</span>!</>
                        ) : savingsAmount > 0 ? (
                          <><Target className="w-4 h-4 text-primary" />
                            {monthStatus === 'current'
                              ? <>Try to reduce upcoming expenses by <span className="text-primary font-bold">{formatCurrency(savingsLimit - savingsAmount, settings)}</span> to hit your target.</>
                              : <>You missed your target by <span className="text-primary font-bold">{formatCurrency(savingsLimit - savingsAmount, settings)}</span>.</>
                            }
                          </>
                        ) : (
                  ...
                  <div className="text-sm text-muted-foreground mt-1">
                    {effectiveIncome > 0 ? (
                      <div className="flex items-center gap-2">
                        <span>{monthStatus === 'current' ? 'Spent so far: ' : 'Total spent: '} <strong className={isActuallyOver ? "text-destructive" : "text-foreground"}>{formatCurrency(totalSpent, settings)}</strong></span>
                      </div>
                    ) : "No income recorded."}
                  </div>
                </div>
                  ...
                  <div className="hidden sm:flex flex-col items-end relative">
                    <AnimatePresence>
                      {!isSimpleView && extraSavingsAmount > 0 && (
                        ...
                        >
                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[hsl(var(--stat-income-bg))] border border-[hsl(var(--stat-income-text))/0.2]">
                            <TrendingUp className="w-3 h-3 text-[hsl(var(--stat-income-text))]" />
                            <span className="text-xs font-medium text-[hsl(var(--stat-income-text))]">Efficiency: +{formatCurrency(extraSavingsAmount, settings)}</span>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                      <PopoverTrigger asChild>
                        <button className="flex items-center gap-1 text-xs hover:text-primary transition-colors outline-none">
                          <Target size={14} />
                          <span>Goals</span>
                        </button>
                      </PopoverTrigger>
