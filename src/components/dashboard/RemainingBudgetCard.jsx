      ...
      <CardContent className="p-5 flex-1 flex flex-col">
        <div className="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
          ...
          <div className="flex flex-col-reverse sm:flex-row items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <div className="flex items-center gap-2" data-tutorial="desktop-actions">
              {importDataButton}
              {addExpenseButton}
                  ...
                  {isTotalOver ? (
                    ...
                    <div className="space-y-1">
                      <h2 className="text-4xl font-extrabold text-foreground flex items-center gap-2 tracking-tight">
                        {Math.round(savingsPctDisplay)}% <span className="text-xl font-semibold text-[hsl(var(--stat-income-text))]">{monthStatus === 'current' ? "Projected to save" : "Saved"}</span>
                      </h2>
                  ...
                  <div className="text-sm text-muted-foreground mt-1">
                    {effectiveIncome > 0 ? (
                      <div className="flex items-center gap-2">
                        <span>{monthStatus === 'current' ? 'Spent so far: ' : 'Total spent: '} <strong className={isTotalOver ? "text-destructive" : "text-foreground"}>{formatCurrency(totalSpent, settings)}</strong></span>
                      </div>
                    ) : "No income recorded."}
                  </div>
                </div>
