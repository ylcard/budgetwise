...
import { getMonthBoundaries, getFirstDayOfMonth, getLastDayOfMonth, formatDate } from "../utils/dateUtils";
 ...
 * Visualizes a 3-month financial horizon:
 * 1. Last Month (Actuals)
 * 2. This Month (Projected based on current date)
 * 3. Next Month (Target based on 6-month historical average)
 *
 * @param {Object} props
...
export default function ProjectionChart({ settings }) {
  ...
  const horizonWindow = useMemo(() => {
    return {
      from: getFirstDayOfMonth(today.getMonth() - 6, today.getFullYear()),
      to: getLastDayOfMonth(today.getMonth(), today.getFullYear())
    };
  }, [today]);
  ...
  const { data, sixMonthAvg } = useMemo(() => {
    ...
    let validMonths = 0;
    for (let i = 1; i <= 6; i++) {
      // dateUtils handles month rollover automatically (e.g., month - 1)
      const bounds = getMonthBoundaries(today.getMonth() - i, today.getFullYear());
    ...
    const lastBoundaries = getMonthBoundaries(today.getMonth() - 1, today.getFullYear());
    // We use the start string to format the label (e.g., "Jan")
    const lastMonthLabel = formatDate(lastBoundaries.monthStart, 'MMM');
    ...
    const chartData = [
      { label: 'Last Month', subLabel: lastMonthLabel, income: lastIncome, expense: lastExpenses, type: 'past' },
