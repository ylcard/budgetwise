...
export default memo(function FinancialHealthScore({ healthData, className }) {
  ...
  if (!healthData) {
    return (
      <div className="w-full overflow-hidden" ref={emblaRef}>
        <div className="flex px-4 md:px-0 gap-3 md:grid md:grid-cols-5 animate-pulse">
          <div className="h-24 bg-gray-100 rounded-xl border border-gray-200 flex-[0_0_90%] md:col-span-5" />
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-32 bg-gray-50 rounded-lg border border-gray-100 flex-[0_0_45%] md:col-span-1" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full overflow-hidden py-1" ref={emblaRef}>
      <div className={`flex touch-pan-y px-4 md:px-0 gap-3 md:gap-4 md:grid md:grid-cols-5 md:auto-rows-max ${className || ''}`}>
        
        {/* Main Header Card - Hero Slide on Mobile, Span 5 on Desktop */}
        <div className={`flex-[0_0_90%] min-w-0 md:col-span-5 relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 md:p-5 rounded-xl border shadow-md transition-colors duration-300 gap-4 ${headerStyle.bg} ${headerStyle.border}`}>
        ...
        <div className="flex-[0_0_45%] min-w-0 md:col-span-1">
