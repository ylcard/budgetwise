...
export default function BudgetPostMortem({ budget, transactions, categories, settings }) {
  const analysis = useMemo(() => {
    ...
    const variance = new Decimal(totalSpent).minus(planned).toDecimalPlaces(2).toNumber();
    const variancePercentage = planned > 0 ? new Decimal(variance).dividedBy(planned).times(100).toDecimalPlaces(2).toNumber() : 0;
    ...
    budgetTransactions.forEach(t => {
      ...
      if (cat) {
        ...
        categoryBreakdown[catName] = new Decimal(categoryBreakdown[catName] || 0)
          .plus(t.amount)
          .toNumber(); // Keep precision inside breakdown object
    ...
    if (isCompleted) {
      ...
      if (variancePercentage > 10) {
        ...
        const spentPct = new Decimal(totalSpent).dividedBy(planned).times(100).toNumber();
        insights.push({
          type: 'info',
          message: `Spent ${spentPct.toFixed(0)}% of budget so far.`
        });
      }
    }

    if (largestCategory) {
      const catPercentage = new Decimal(largestAmount).dividedBy(totalSpent).times(100).toNumber();
      if (catPercentage > 40) {
        insights.push({
          type: 'info',
