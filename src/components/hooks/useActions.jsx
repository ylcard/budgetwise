...
import { parseDate, getMonthBoundaries, normalizeToMidnight } from "../utils/dateUtils";
 ...
 * Includes optimistic updates and budget synchronization logic.
 * @param {Object} [options] - Optional configuration.
 * @param {Function} [options.onSuccess] - Callback after successful transaction creation.
 */
...
export const useGoalActions = (user, goals) => {
  ...
  const updateGoalMutation = useMutation({
    ...
    onSuccess: async (_, variables) => {
      ...
      await queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.BUDGET_GOALS] });
...
export const useSavingsGoalActions = () => {
  ...
  const createMutation = useCreateEntity({
    entityName: 'Goal',
    queryKeysToInvalidate: [QUERY_KEYS.SAVINGS_GOALS],
    onBeforeCreate: async (data) => ({
      ...data,
      user_email: user.email,
      virtual_balance: 0,
