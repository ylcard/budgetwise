 ...
 * Seeds default 50/30/20 goals if none exist for the user.
...
/**
 * Hook to fetch transactions, optionally filtered by date range.
  ...
  const { data: transactions = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      if (startDate && endDate) {
        ...
        // Used local-safe subDays and formatting
        const bufferStartDate = formatDateString(subDays(normalizeToMidnight(startDate), 30));
...
/**
 * Specialized hook for fetching historical income transactions for projections.
 * Fetches the last 180 days of income data.
 */
export const useHistoricalIncomeTransactions = (user) => {
  const { data: incomeTransactions = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const startDate = formatDateString(subDays(normalizeToMidnight(new Date()), 180));
      return await fetchWithRetry(() => base44.entities.Transaction.filter({
        type: 'income',
        date: { $gte: startDate }
      }));
...
/**
 * Hook for orchestrating system budget creation and updates.
 * Ensures 50/30/20 budgets exist for the selected period.
 */
