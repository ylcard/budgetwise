...
export function useRuleActions() {
  ...
  const createRule = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Rule created", {
        description: "Transactions will be categorized automatically."
      });
    }
  });
  ...
  const deleteBulkRules = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Rules deleted", {
        description: "Selected automation rules have been removed."
      });
    },
    onError: (e) => toast.error("Failed to delete rules", {
  ...
  const handleRemoveKeyword = (rule, keywordToRemove) => {
    ...
    if (!updatedKeywords) {
      toast.error("Cannot remove last keyword");
      return;
    }
    updateRule.mutate({ id: rule.id, data: { keyword: updatedKeywords } });
  };
  ...
  const handleSaveRule = (e) => {
    if (e && e.preventDefault) e.preventDefault();
    if (!formData.categoryId) return toast.error("Missing Category");

    // Validation
    if (isRegexMode) {
      if (!formData.regexPattern) return toast.error("Missing Pattern");
      try { new RegExp(formData.regexPattern); }
      catch (e) {
        return toast.error("Invalid Regex", {
          description: e.message
        });
      }
    } else {
      if (!formData.keyword) return toast.error("Missing Keywords");
    }
