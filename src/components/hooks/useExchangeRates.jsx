  ...
  const refreshRates = async (sourceCurrency, targetCurrency, date, force = false) => {
    ...
    try {
      ...
      for (const [currency, rate] of Object.entries(response.rates)) {
        ...
        // Previous logic, fixing for eToro
        //const isNowFresh = areRatesFresh(currentRates, currency, 'USD', date, 14);
        const isNowFresh = areRatesFresh(currentRates, currency, 'EUR', date, 14);

        if (isNowFresh) continue;

        // INVERSION LOGIC: External APIs return 1 EUR = X USD. 
        // We store 1 USD = (1/X) EUR to maintain app-wide multiplication logic. (eToro)
        const invertedRate = parseFloat((1 / rate).toFixed(6));
        ...
        if (existingRate) {
          ...
          if (Math.abs(existingRate.rate - rate) > 0.0001) {
              ...
              */
          if (Math.abs(existingRate.rate - invertedRate) > 0.0001) {
            ratesToUpdate.push({ id: existingRate.id, rate: invertedRate });
          }
        } else {
