...
const BudgetHealthCircular = ({ budget, transactions, settings }) => {
  ...
  const getCircleColor = (budget) => {
    if (budget.systemBudgetType === 'needs') return 'stroke-primary';
    if (budget.systemBudgetType === 'wants') return 'stroke-secondary';
  ...
  return (
    <div
      className={`bg-card h-full rounded-2xl p-4 border border-border shadow-sm flex flex-col items-center justify-between min-h-[180px]`}
    >
      ...
      <div className="w-full relative flex justify-center mb-2 px-1">
        <Link
          ...
          className="text-foreground font-semibold text-sm text-center truncate max-w-[85%] cursor-pointer hover:opacity-80 transition-opacity"
      ...
      <div className="w-full text-center">
        <div className="text-foreground text-xl font-bold mb-1 truncate">
        ...
        {budget.paidPrior > 0 && (
          <p className="text-muted-foreground/50 text-[9px] mt-0.5 italic">
            Inc. {formatCurrency(budget.paidPrior, settings)} paid prior
          </p>
        )}
      </div>
    </div>
