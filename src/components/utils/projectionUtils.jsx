...
export const calculateProjection = (transactions, categories, historicalMonths = 6) => {
  ...
  relevantTransactions.forEach(t => {
    ...
    if (monthKeys.includes(monthKey)) {
      categoryHistory[catId][monthKey] = new Decimal(categoryHistory[catId][monthKey] || 0)
        .plus(t.amount || 0).toNumber();
    }
  });

  // 2. Calculate Adjusted Average per Category
  let decTotalProjectedMonthly = new Decimal(0);
  ...
  Object.keys(categoryHistory).forEach(catId => {
    ...
    decTotalProjectedMonthly = decTotalProjectedMonthly.plus(adjustedAvg);
  ...
  return {
    totalProjectedMonthly: decTotalProjectedMonthly.toNumber(),
...
export const estimateCurrentMonth = (currentMonthTransactions, safeMonthlyBaseline) => {
  ...
  return {
    ...
    total: new Decimal(actualSpent).plus(projectedRemaining).toNumber()
  };
};