...
import { parseDate, normalizeToMidnight, formatDateString } from "./dateUtils";
...
export function analyzeEventPatterns(transactions, categories) {
  ...
  // Use normalizeToMidnight for date comparisons
  const now = new Date();

  const categoryMap = createEntityMap(categories);

  // Filter only expenses and sort by date
  // Use parseDate to ensure consistent sorting
  const expenses = transactions
    .filter(t => t.type === 'expense' && t.isPaid && (parseDate(t.paidDate || t.date) < now))
    .sort((a, b) => parseDate(a.paidDate || a.date) - parseDate(b.paidDate || b.date));
  ...
  const dailyGroups = {};
  expenses.forEach(t => {
    const date = formatDateString(t.paidDate || t.date); // Returns YYYY-MM-DD
    if (!dailyGroups[date]) {
      dailyGroups[date] = [];
    }
  ...
  while (i < dailyData.length) {
    ...
    if (current.total > threshold) {
      ...
      // Helper for day diff
      const getDayDiff = (d1, d2) => Math.round((d1 - d2) / (1000 * 60 * 60 * 24));

      while (j < dailyData.length &&
        getDayDiff(dailyData[j].date, current.date) <= 7) {

        const lastClusterDay = cluster[cluster.length - 1];

        // Replaced differenceInDays with custom helper
        const daysSinceLastActive = getDayDiff(dailyData[j].date, lastClusterDay.date);

        // BREAK if there is a gap of more than 2 days (Separate weekends = Separate trips)
      ...
      if (cluster.length >= 2) {
        ...
        const eventType = inferEventType(allTransactions, categoryMap, cluster.length);
        ...
        events.push({
          ...
          duration: getDayDiff(cluster[cluster.length - 1].date, cluster[0].date) + 1,
          totalAmount,
...
function inferEventType(transactions, categoryMap, durationDays) {
  ...
  // Ensure hasTransport and hasDining are actually defined
  const hasTransport = categoryNames.some(n => n.includes('transport') || n.includes('travel') || n.includes('train') || n.includes('rail')) ||
  ...
  // 1. The Crown Jewel Rule: If there is a ticket, it's a Concert/Event Trip.
  // We use duration to subtype it if needed, but the CORE is the event.
  if (hasTicket) {
...
export function analyzeExpenseElasticity(transactions, categories, systemBudgets) {
  ...
  transactions.forEach(t => {
    ...
    const date = parseDate(t.paidDate || t.date);
...
/**
 * Calculate average spending per category from a list of months
 * @param {Array} months - List of monthly data objects
...
/**
 * Calculate confidence score for a budget archetype
 * @param {Array} events - List of events of this type
 */
