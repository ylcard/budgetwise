...
export function calculateNextOccurrence(recurring, fromDate = new Date()) {
  const { frequency, dayOfMonth, dayOfWeek, startDate, endDate, lastProcessedDate, nextOccurrence } = recurring;

  // GUARD: If startDate is missing (e.g., during a partial form update),
  // we cannot recalculate. Safely return the existing nextOccurrence.
  if (!startDate) {
