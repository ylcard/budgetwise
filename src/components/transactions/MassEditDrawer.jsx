...
import { Tag, Calendar, Wallet, CheckCircle2, Shield, Plus, X, Search, Check, ChevronsUpDown, Hash } from 'lucide-react';
...
/**
 * MassEditDrawer Component
 * Allows bulk editing of transaction properties. Supports both Desktop (Dialog) and Mobile (Drawer).
 * @param {Object} props
 * @param {boolean} props.open - Controlled visibility state.
 * @param {Function} props.onOpenChange - State setter.
 * @param {number} props.selectedCount - Count of items being edited.
 * @param {Function} props.onSave - Callback with updates { field: value }.
 */
export function MassEditDrawer({
  ...
  const onSubmit = (data) => {
    ...
    if (activeFields.includes('isPaid')) {
      ...
      if (updates.isPaid) {
        // Use local date string to prevent timezone offset issues
        updates.paidDate = formatDateString(new Date());
      }
    }
