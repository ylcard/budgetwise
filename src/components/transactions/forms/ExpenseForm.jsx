                    ...
                    <div className="flex items-center text-left">
                      {budget.isSystemBudget ? (
                        ...
                        <span className={cn(
                          ...
                          budget.status === 'active' ? 'bg-[hsl(var(--status-paid-text))]' : 'bg-muted-foreground'
                        )} />
                      )}
                      <div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4 pt-4 space-y-4 md:px-0 md:overflow-visible">
          ...
          <Alert variant="warning" className="bg-warning/10 border-warning/20">
            ...
            <div className="ml-2">
              ...
              <div className="mt-2 flex gap-2">
                <button type="button" onClick={() => onCancel()} className="text-xs underline text-warning font-bold brightness-75">Cancel</button>
                <button type="button" onClick={() => setPotentialDuplicate(null)} className="text-xs underline text-warning">Ignore & Create Anyway</button>
              </div>
            </div>
          </Alert>
        )}
        ...
        <div className="flex gap-2 items-end">
          ...
          <CustomButton
            ...
            className="text-primary h-10 w-10 shrink-0 hover:bg-primary/10"
          >
                ...
                {isForeignCurrency && (
                  ...
                  >
                    {(() => {
                      ...
                      return <span className="text-[10px] text-warning font-bold uppercase">Rate Missing</span>;
                    })()}
                    <button type="button" onClick={handleRefreshRates} className="text-[10px] text-primary font-bold hover:underline">SYNC</button>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <CustomButton
                ...
                className={cn(
                  ...
                  formData.isPaid && !formData.isCashExpense ? "bg-primary hover:bg-primary/90 text-primary-foreground border-solid" : "text-muted-foreground hover:text-primary hover:border-primary/50"
                )}
                title="Mark as Paid"
              >
                <CheckCircle2 className={cn("h-5 w-5", formData.isPaid && !formData.isCashExpense ? "text-primary-foreground" : "text-primary")} />
              ...
              <CustomButton
                ...
                onClick={toggleCash}
                className={cn("h-12 w-12 md:w-auto px-0 md:px-3", formData.isCashExpense ? "bg-[hsl(var(--status-paid-text))] hover:brightness-95 text-white" : "text-muted-foreground hover:text-[hsl(var(--status-paid-text))] hover:border-[hsl(var(--status-paid-text))/0.5] hover:bg-accent")}
                title="Paid in Cash"
              >
                <Banknote className={cn("h-5 w-5", formData.isCashExpense ? "text-white" : "text-[hsl(var(--status-paid-text))]")} />
            ...
            <div className="flex gap-3">
              ...
              <CustomButton
                ...
                className={cn(
                  ...
                  formData.financial_priority ? "bg-primary/5 border-primary/20 border-solid" : "text-muted-foreground hover:bg-accent"
                )}
                onClick={() => {
              ...
              >
                {formData.financial_priority ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-foreground">{FINANCIAL_PRIORITIES[formData.financial_priority].label}</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2">
                            ...
                            >
                              ...
                              <div className="flex items-center text-sm">
                                {budget.isSystemBudget ? (
                                  ...
                                  <span className={`w-2 h-2 rounded-full mr-2 ${budget.status === 'active' ? 'bg-[hsl(var(--status-paid-text))]' : 'bg-muted-foreground'}`} />
                                )}
                                {getBudgetDisplayName(budget)}
                                {budget.isSystemBudget && <span className="ml-1 text-xs text-muted-foreground/70">({formatDate(budget.startDate, 'MMM')})</span>}
                              </div>
                            </CommandItem>
                          ))}
                        </CommandGroup>
