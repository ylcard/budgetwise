  ...
  const renderCurrencySelector = () => {
    ...
    const TriggerButton = (
      <CustomButton
        ...
        role="combobox"
        aria-expanded={open}
        className="h-full rounded-r-none border-r border-border px-3 hover:bg-accent text-muted-foreground font-medium"
      >
              ...
              <CommandGroup className={mobile ? "overflow-y-auto" : "max-h-64 overflow-y-auto [&::-webkit-scrollbar]:hidden"}>
                {currencies.map((c) => (
                  <CommandItem
                    ...
                    className={`flex items-center gap-2 px-3 py-2 cursor-pointer ${currency === c.code ? 'bg-primary/10 text-primary' : ''} ${mobile ? 'py-3' : ''}`}
                  >
                    <span className="w-4 text-center text-muted-foreground font-medium text-xs">{c.symbol}</span>
