...
import { Trash2, ArrowUpDown, ArrowUp, ArrowDown, RefreshCw, Check, Wallet, ShieldCheck, Sparkles, Info } from "lucide-react";
...
import { parseDate, formatDate, getRelativeDateLabel } from "@/components/utils/dateUtils";
import DuplicateResolver from "@/components/transactions/DuplicateResolver";

/**
 * CategorizeReview Component
 * Allows users to review, edit, categorize, and assign budgets to imported transactions
 * before they are permanently added to the database.
 *
 * @param {Object} props
 * @param {Array} props.data - Array of imported transaction objects
 * @param {Array} props.categories - Available categories
 * @param {Array} props.allBudgets - Available budgets (System + Custom)
 * @param {Function} props.onUpdateRow - Handler to update a specific row
 * @param {Function} props.onDeleteRows - Handler to delete rows
 */
export default function CategorizeReview({ data, categories, allBudgets = [], onUpdateRow, onDeleteRows }) {
  ...
  /**
   * Generates the label for the System Budget option based on transaction date.
   * Format: "Needs (Jan)" or "Needs (Jan 2024)" if not current year.
   */
  const getSystemBudgetLabel = (priority, dateStr, isPaid, paidDate) => {
    const effectiveDate = (isPaid && paidDate) ? paidDate : dateStr;
    const d = parseDate(effectiveDate);
    if (!d) return '';

    const isCurr = d.getFullYear() === new Date().getFullYear();
    const datePart = formatDate(d, isCurr ? "MMM" : "MMM yyyy");
    const priorityLabel = FINANCIAL_PRIORITIES[priority]?.label || 'System Budget';

    return `${priorityLabel} (${datePart})`;
  };
        ...
        <div className="block md:hidden space-y-4">
          {sortedData.map((row) => (
            row.isDuplicate && row.duplicateMatch ? (
              <DuplicateResolver
                key={`dup-${row.originalIndex}`}
                newTx={row}
                survivorTx={row.duplicateMatch}
                onKeepSurvivor={() => onDeleteRows([row.originalIndex])}
                onKeepBoth={() => onUpdateRow(row.originalIndex, { isDuplicate: false, duplicateMatch: null })}
                onOverwrite={() => onDeleteRows([row.originalIndex])}
              />
            ) : (
              <div
                ...
                <div className="space-y-2 pt-2 border-t border-gray-100">
                  ...
                  <MobileDrawerSelect
                    ...
                    options={[
                      {
                        value: "system",
                        label: `System: ${getSystemBudgetLabel(row.financial_priority, row.date, row.isPaid, row.paidDate)}`
                      },
                      ...
                      {row.type === 'expense' ? (
                        <MobileDrawerSelect
                          ...
                          options={[
                            {
                              value: "system",
                              label: getSystemBudgetLabel(row.financial_priority, row.date, row.isPaid, row.paidDate)
                            },
