...
import { useCategoryRules, useAllBudgets, useGoals, useTransactions } from "@/components/hooks/useBase44Entities";
...
export default function ImportWizard({ onSuccess }) {
  ...
  // Optimized Reactive View
  const enrichedData = useMemo(() => {
    if (!processedData.length) return [];

    // 1. Create a "Lookup Index" for O(1) speed
    // Key format: "amount|date" (e.g., "42.50|2024-05-20")
    const dbIndex = new Map();
    existingTransactions?.forEach(ex => {
      const key = `${Math.abs(ex.amount).toFixed(2)}|${ex.date}`;
      dbIndex.set(key, ex);
    });

    // 2. Map the data using the index
    return processedData.map(item => {
      const key = `${Math.abs(item.amount).toFixed(2)}|${item.date}`;
      const survivor = dbIndex.get(key);
  ...
  }, [processedData, existingTransactions]);
  ...
  const handleFileSelect = async (selectedFile) => {
    ...
    if (selectedFile.name.toLowerCase().endsWith('.pdf') || selectedFile.type === 'application/pdf') {
      ...
      toast("File parsed", { description: `Found ${parsed.data.length} rows.` });

      // Set rawData early so useTransactions starts fetching immediately
      setRawData(parsed.data.map(row => ({ date: row[mappings.date] })));
    }
  };
  ...
  const handlePdfProcessing = async (file) => {
    ...
    setError(null);
    try {
      toast("Uploading...", { description: "Uploading file for analysis." });

      // 1. Upload File
      const { file_url } = await fetchWithRetry(() => base44.integrations.Core.UploadFile({ file: file }));

      toast("Analyzing...", { description: "Extracting data from PDF. This may take a moment." });
      ...
      */
      ...
      toast("Categorizing...", { description: "AI is cleaning and organizing..." });
      ...
      setStep(2);
      toast.success("Success", { description: `Extracted ${processed.length} transactions.` });
    } catch (error) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (goalsLoading || !goals.length) {
        toast("System warming up", { description: "Still loading your goals. Please try again in a second." });
      ...
      const transactionsToCreate = dataToImport.map(item => {
        ...
        // Logic now relies on useMergedCategories being up to date. 
        // If item.categoryId is null, we check the global list provided by the hook.
        if (item.type === 'expense' && !finalCatId) {
          const match = categories.find(c => c.name.toLowerCase() === (item.category || '').toLowerCase());
          finalCatId = match ? match.id : null;
        }
